...
def respond2(history,key):
    ...
    for i in history[-10:]:
        prompt_claude+=f'{HUMAN_PROMPT} {history[-1][0]}'
        if i[1] is not None:
            prompt_claude+=f'{AI_PROMPT} {history[-1][1]}'
...
def onload(request: gr.Request):
    ...
    return txt1,txt2,txt3,request.query_params["id"],txtTitle,txtDes
...
def sendAll(history1,history2,history3,key1,key2,key3):
    ...
    print(h1[0])
    return h1[0],history2,history3#,respond2(history2,key2),respond3(history3,key3)
async def sendAll_in_parallel(chatbot1,chatbot2,chatbot3,pwd1,pwd2,pwd3):
    a=await Promise.all([
            ...
            ])#.then(lambda results: [list(results[0])[0], list(results[1])[0], list(results[2])[0]])
    #print(a)
    return a
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=300):
            ...
            msg1.submit(user, [msg1, chatbot1], [msg1, chatbot1], queue=False).then(respond1, [chatbot1,pwd1], [chatbot1])#.then(UpdateOthers, [chatbot,text1,text1,tbTitle,tbDes], [chatbot,text1chg,text1chg])
        ...
        with gr.Column(scale=1, min_width=300):
            ...
            msg2.submit(user, [msg2, chatbot2], [msg2, chatbot2], queue=False).then(respond2, [chatbot2,pwd2], [chatbot2])#.then(UpdateOthers, [chatbot,text1,text2,tbTitle,tbDes], [chatbot,text1chg,text2chg])
        ...
        with gr.Column(scale=1, min_width=300):
            ...
            msg3.submit(user, [msg3, chatbot3], [msg3, chatbot3], queue=False).then(respond3, [chatbot3,pwd3], [chatbot3])#.then(UpdateOthers, [chatbot,text1,text2,tbTitle,tbDes], [chatbot,text1chg,text2chg])
    ...
    msg0.submit(askAll,inputs=[msg0,msg1,msg2,msg3,chatbot1,chatbot2,chatbot3],outputs=[msg0,msg1,msg2,msg3,chatbot1,chatbot2,chatbot3],queue=False).then(respond1, [chatbot1,pwd1], [chatbot1]).then(respond3, [chatbot3,pwd3], [chatbot3])#.then(sendAll,inputs=[chatbot1,chatbot2,chatbot3,pwd1,pwd2,pwd3],outputs=[chatbot1,chatbot2,chatbot3])
    ...
    #demo.load(onload,inputs=None,outputs=[text1,text2,text3,wId,tbTitle,tbDes])
